...
export default function LandingPage() {
  ...
  const [werVisible, setWerVisible]     = useState(false)
  const benchmarkRef                    = useRef<HTMLDivElement>(null)
  const werRef                          = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    ...
    const werObs = new IntersectionObserver(
      ([e]) => { if (e.isIntersecting) setWerVisible(true) },
      { threshold: 0.25 }
    )
    if (werRef.current) werObs.observe(werRef.current)
    return () => { window.removeEventListener('scroll', fn); obs.disconnect(); werObs.disconnect() }
  }, [])
  ...
  useEffect(() => {
    ...
    const drawFrame = (idx: number) => {
      ...
      // Find the target frame; if not yet loaded, walk backward to the nearest loaded one
      let img = images[clamped]
      let drawIdx = clamped
      if (!img?.complete || img.naturalWidth === 0) {
        for (let b = clamped - 1; b >= 0; b--) {
          if (images[b]?.complete && images[b].naturalWidth > 0) { img = images[b]; drawIdx = b; break }
        }
      ...
      // Only mark currentFrame as the intended target so it retries when the real frame loads
    ...
    <>
      ...
      <style suppressHydrationWarning>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .wer-fill-arctan {
          animation: wer-arctan-shimmer 2.8s ease-in-out infinite;
          animation-play-state: paused;
        }
        .wer-fill-arctan.wer-ready {
          animation-play-state: running;
        }
        @keyframes wer-arctan-shimmer {
          0%, 100% { opacity: 1; }
          50%       { opacity: 0.72; }
        }
        .wer-bar-brand {
...
.wer-factors {
        ...
        @media (max-width: 768px) {
          ...
          .rsp-contact-btns a,
          .rsp-contact-btns button { justify-content: center !important; text-align: center !important; }
          ...
          .rsp-stream-svg     { overflow: hidden !important; }
          .rsp-stream-svg > div { min-width: 0 !important; width: 100% !important; }
      ...
      <div style={{ fontFamily: GTA, color: D.ink, background: D.white }}>
        ...
        <nav style={{
          ...
          <div className="rsp-nav-inner" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 40px', height: '100%', display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center' }}>
            {/* Left: logo */}
        ...
        <section id="wer" ref={werRef} className="rsp-wer-section" style={{ background: D.offWhite, padding: '100px 40px' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            ...
            {(() => {
              const MAX = 20
              const rows = [
                { brand: 'Arctan',      wer: 13.3, logo: `${BP}/logos/arctan-mark.svg`,     isArctan: true  },
                { brand: 'ai-coustics', wer: 15.9, logo: `${BP}/logos/aicoustics-mark.png`,  isArctan: false },
                { brand: 'Krisp',       wer: 16.7, logo: `${BP}/logos/krisp-mark.png`,       isArctan: false },
              ]
              return (
                <div>
                  ...
                  <div className="wer-bars">
                    {rows.map(({ brand, wer, logo, isArctan }) => (
                      <div key={brand} className="wer-bar-row">
                        <div className="wer-bar-brand" style={{ display: 'flex', alignItems: 'center', gap: 8, color: isArctan ? D.teal : D.muted }}>
                          <img src={logo} alt={brand} width="22" height="22" style={{ display: 'block', borderRadius: 5, flexShrink: 0, objectFit: 'contain' }} />
                          {brand}
                        </div>
                        <div className="wer-bar-track">
                          <div
                            className={`wer-bar-fill${isArctan ? ` wer-fill-arctan${werVisible ? ' wer-ready' : ''}` : ''}`}
                            style={{ width: werVisible ? `${(wer / MAX) * 100}%` : '0%', background: isArctan ? D.teal : 'rgba(27,6,36,0.14)' }}
                          />
                        </div>
        ...
        {/* ══════ SDK / INTEGRATION ══════ */}
        <section id="integration" className="rsp-intg-section" style={{ background: D.offWhite, padding: '100px 40px' }}>
          <div className="rsp-intg-grid" style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
            <div>
              <h2 className="rsp-intg-h2" style={{ fontFamily: GTA, fontSize: 40, fontWeight: 400, letterSpacing: '-0.05em', lineHeight: '1.1em', fontFeatureSettings: '"blwf" on,"cv09" on,"cv03" on,"cv04" on,"cv11" on', color: D.ink, marginBottom: 24 }}>
                <span style={{ color: '#1A8A70' }}>Native</span> in the language<br/>you ship in.
              </h2>
              <p style={{ fontFamily: GTA, fontSize: 18, fontWeight: 400, letterSpacing: '-0.01em', lineHeight: '1.65em', color: D.inkSub, marginBottom: 36 }}>
                First-party SDKs for Node, Python, Rust, and C++.
              </p>
              <div style={{ display: 'flex', gap: 64, flexWrap: 'wrap' as const, marginBottom: 36 }}>
                <img src={`${BP}/logos/livekit-logo.png`} alt="LiveKit" style={{ display: 'block', width: 112, height: 'auto', objectFit: 'contain' }} />
                <img src={`${BP}/logos/pipecat-logo.png`} alt="Pipecat" style={{ display: 'block', width: 150, height: 'auto', objectFit: 'contain' }} />
              </div>
            </div>

            {/* Code block */}
            <div className="rsp-code-wrap" style={{ borderRadius: 16, overflow: 'hidden', border: `1px solid ${D.border}`, background: D.white }}>
              <div style={{ padding: '12px 20px', borderBottom: `1px solid ${D.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', gap: 6 }}>
                  {[0,1,2].map(i => <div key={i} style={{ width: 10, height: 10, borderRadius: '50%', background: 'rgba(27,6,36,0.12)' }}/>)}
                </div>
                <span style={{ fontFamily: "'Fragment Mono', monospace", fontSize: 11, color: D.muted }}>Python</span>
              </div>
              <div className="rsp-code-inner" style={{ padding: '28px', fontFamily: "'Fragment Mono', monospace", fontSize: 13, lineHeight: '1.9em', minWidth: 'max-content' }}>
                <div><span style={{ color: '#7C3AED' }}>from</span> <span style={{ color: '#1A56DB' }}>arctan</span> <span style={{ color: '#7C3AED' }}>import</span> <span style={{ color: '#B45309' }}>arctan_enhancer</span></div>
                <div style={{ marginTop: 16 }}/>
                <div><span style={{ color: '#047857' }}>await</span> <span style={{ color: 'rgba(27,6,36,0.75)' }}>session.start(</span></div>
                <div style={{ paddingLeft: 24 }}><span style={{ color: 'rgba(27,6,36,0.5)' }}>agent</span><span style={{ color: 'rgba(27,6,36,0.35)' }}>=</span><span style={{ color: '#1A56DB' }}>Assistant</span><span style={{ color: 'rgba(27,6,36,0.75)' }}>(),</span></div>
                <div style={{ paddingLeft: 24 }}><span style={{ color: 'rgba(27,6,36,0.5)' }}>room</span><span style={{ color: 'rgba(27,6,36,0.35)' }}>=</span><span style={{ color: 'rgba(27,6,36,0.75)' }}>ctx.room,</span></div>
                <div style={{ paddingLeft: 24 }}><span style={{ color: 'rgba(27,6,36,0.5)' }}>room_options</span><span style={{ color: 'rgba(27,6,36,0.35)' }}>=</span><span style={{ color: '#1A56DB' }}>room_io.RoomOptions</span><span style={{ color: 'rgba(27,6,36,0.75)' }}>(</span></div>
                <div style={{ paddingLeft: 48 }}><span style={{ color: 'rgba(27,6,36,0.5)' }}>audio_input</span><span style={{ color: 'rgba(27,6,36,0.35)' }}>=</span><span style={{ color: '#1A56DB' }}>room_io.AudioInputOptions</span><span style={{ color: 'rgba(27,6,36,0.75)' }}>(</span></div>
                <div style={{ paddingLeft: 72 }}><span style={{ color: 'rgba(27,6,36,0.5)' }}>noise_cancellation</span><span style={{ color: 'rgba(27,6,36,0.35)' }}>=</span><span style={{ color: '#1A56DB' }}>arctan_enhancer</span><span style={{ color: 'rgba(27,6,36,0.75)' }}>.</span><span style={{ color: '#B45309' }}>noise_canceller</span><span style={{ color: 'rgba(27,6,36,0.75)' }}>(</span></div>
                <div style={{ paddingLeft: 96 }}><span style={{ color: 'rgba(27,6,36,0.5)' }}>key</span><span style={{ color: 'rgba(27,6,36,0.35)' }}>=</span><span style={{ color: '#1A56DB' }}>os.environ</span><span style={{ color: 'rgba(27,6,36,0.75)' }}>[</span><span style={{ color: '#047857' }}>"ARCTAN_KEY"</span><span style={{ color: 'rgba(27,6,36,0.75)' }}>]</span></div>
                <div style={{ paddingLeft: 72 }}><span style={{ color: 'rgba(27,6,36,0.75)' }}>),</span></div>
                <div style={{ paddingLeft: 48 }}><span style={{ color: 'rgba(27,6,36,0.75)' }}>),</span></div>
                <div style={{ paddingLeft: 24 }}><span style={{ color: 'rgba(27,6,36,0.75)' }}>),</span></div>
                <div><span style={{ color: 'rgba(27,6,36,0.75)' }}>)</span></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="contact" className="rsp-contact-section" style={{ background: D.white, padding: '120px 40px', borderTop: `1px solid ${D.border}` }}>
          <div style={{ maxWidth: 600, margin: '0 auto', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', textAlign: 'center' as const }}>
            ...
            {/* Logo */}
            <img src={`${BP}/logos/arctan-mark.svg`} alt="Arctan" width="99" height="59" style={{ display: 'block', marginBottom: 36 }} />

            {/* Heading */}
            <h2 className="rsp-contact-h2" style={{ fontFamily: GTA_MD, fontSize: 52, fontWeight: 500, letterSpacing: '-0.05em', lineHeight: '1.05em', color: D.ink, marginBottom: 20 }}>
              Ready to ship Voice AI<br/>that works?
            </h2>

            {/* Subtext */}
            <p style={{ fontFamily: GTA, fontSize: 18, fontWeight: 400, letterSpacing: '-0.01em', lineHeight: '1.65em', color: D.inkSub, marginBottom: 44, maxWidth: 440 }}>
            ...
            <div className="rsp-contact-btns" style={{ display: 'flex', gap: 12, flexWrap: 'wrap' as const, justifyContent: 'center' as const }}>
              ...
              <a
                ...
                style={{ padding: '14px 36px', borderRadius: 9999, background: 'transparent', color: D.ink, fontFamily: GTA, fontWeight: 400, fontSize: 16, letterSpacing: '-0.01em', border: `1px solid ${D.border}`, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', whiteSpace: 'nowrap' as const }}
              >
                Get a Demo
              </a>
            </div>
        ...
        {/* ══════ FOOTER ══════ */}
        <footer className="rsp-footer" style={{ background: D.offWhite, borderTop: `1px solid ${D.border}`, padding: '52px 40px 36px' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
